<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      /* ---------- Paywall helpers ---------- */
      function isPaid() { return localStorage.getItem(STORE_KEY) === 'yes'; }
      function setPaid() { localStorage.setItem(STORE_KEY, 'yes'); }
      function clearPaid() { localStorage.removeItem(STORE_KEY); }
      ...
      function render(est) {
        var inp = est.inp;
        if (!inp.sf) {
          $('ce_rangeTotal').textContent = "$0 – $0";
          $('ce_rangePerSf').textContent = "$/SF: $0 – $0";
          $('ce_baseTotal').textContent = "$0";
          $('ce_basePsf').textContent = "$/SF: $0";
          $('ce_siteworkTotal').textContent = "$0 – $0";
          $('ce_siteworkNote').textContent = "Included";
          $('ce_assumptions').textContent = "Region 1.00 • Standard finish • Typical glazing • Single dome";
          $('ce_breakdown').innerHTML = '<tr><td colspan="4" class="_muted" style="text-align:center;padding:18px">Run a calculation to see details.</td></tr>';
          return;
        }
    
        $('ce_rangeTotal').textContent = money(est.totals.low) + " – " + money(est.totals.high);
        $('ce_rangePerSf').textContent = "$/SF: " + money(est.totals.lowPSF) + " – " + money(est.totals.highPSF);
        $('ce_baseTotal').textContent = money(est.base.structureCost);
        $('ce_basePsf').textContent = "$/SF: " + money(est.totals.structurePSFout);
    
        var sw = est.parts.sitework;
        $('ce_siteworkTotal').textContent = money(sw.low) + " – " + money(sw.high);
        $('ce_siteworkNote').textContent = sw.included ? "Included" : "Excluded";
      ...
      whenReady(['ce_printBtn'], function () {
        $('ce_printBtn').addEventListener('click', async function (e) {
